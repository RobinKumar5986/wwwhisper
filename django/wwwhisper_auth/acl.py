...
class AllowedUsersCollection(object):
    ...
    def get(self, location_uuid, user_uuid):
        location = self.locations_collection.get(location_uuid)
        if location is None:
            return None
        user = _find2(User, username=user_uuid)
        if user is None:
            return None
        item = HttpPermission.objects.filter(http_location_id=location.path,
                                             user_id=user.id)
        assert item.count() <= 1
        if item.count() == 0:
            return None
    ...
    def delete(self, location_uuid, user_uuid):
        location = self.locations_collection.get(location_uuid)
        if not location:
            return False
        try:
